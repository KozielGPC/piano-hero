...
interface SongImportProps {
	onImportSong: (songData: SongData) => void;
...
export const SongImport: React.FC<SongImportProps> = ({ onImportSong, onError, onSuccess }) => {
		...
		try {
			...
			if (exportedData.audioUrl && exportedData.audioFileName && exportedData.audioType) {
				try {
					...
					onError('Song imported successfully, but audio file could not be restored. You can upload the audio file manually.');
				}
			...
			if (audioFile) {
				onSuccess(`Successfully imported "${exportedData.name}" with ${editorNotes.length} notes and audio file`);
			} else {
				onSuccess(`Successfully imported "${exportedData.name}" with ${editorNotes.length} notes (no audio file included)`);
			}

		} catch (error) {
			console.error('Import error:', error);
			onError(error instanceof Error ? error.message : 'Failed to import song file');
		}
	};
		...
		<Card elevation={3} sx={{ mb: 3 }}>
			<CardContent>
				...
				<Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
					Import a previously exported song file to continue editing. This will load the song data, notes, and audio file (if available).
				</Typography>
